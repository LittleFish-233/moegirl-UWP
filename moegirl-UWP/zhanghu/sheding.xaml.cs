...
namespace moegirl_UWP.zhanghu
{
    ...
    /// </summary>
    public sealed partial class sheding : Page
    {    
                ...
                this.Invoke(async () =>
                {
                    try
                    {
                        ...
                        try
                        {
                            ...
                            await webview1.InvokeScriptAsync("eval", new string[] { String.Format("document.getElementById('contentSub').style.display='none';") });
                            await webview1.InvokeScriptAsync("eval", new string[] { String.Format("document.getElementById('preferences').style.backgroundColor='transparent';") });
                        }
                        catch { }
        ...
        private void chuli(WebViewNavigationStartingEventArgs args)
        {
            //检查是否用户页
            //检查是否为内链 不用修改元素
            string pattern = @"(index.php)";
            Regex rx = new Regex(pattern);
            string pattern1 = @"(Special:)";
            Regex rx1 = new Regex(pattern1);
            if (args.Uri.AbsoluteUri.StartsWith(daima.huancun.xinxi_wangzhi) || daima.huancun.xinxi_wangzhi.StartsWith(args.Uri.AbsoluteUri) || rx.IsMatch(args.Uri.AbsoluteUri) == true || rx1.IsMatch(args.Uri.AbsoluteUri) == true)
            {
                //检查是否为同一个链接
        ...
        private async void button6_Click(object sender, RoutedEventArgs e)
        {
           try
            {
                await webview1.InvokeScriptAsync("eval", new string[] { String.Format("document.getElementById('prefcontrol').click();") });
            }
            catch { }
        }
