...
namespace moegirl_UWP.zhanghu
{
    ...
    /// </summary>
    public sealed partial class jianjie : Page
    {
        ...
        private void Jianjie_Loaded(object sender, RoutedEventArgs e)
        {
            try
            {
                webview1.Navigate(new Uri(daima.huancun.yonghu_wangzhi + daima.huancun.zhanghao.yonghuming));
            }
            catch
            {
        ...
        private async void webview1_NavigationStarting(WebView sender, WebViewNavigationStartingEventArgs args)
        {
            ...
            switch (daima.huancun.panduan_leixing(args.Uri.AbsoluteUri))
            {
                case 2:
                    ...
                    if (args.Uri.AbsoluteUri.StartsWith(daima.huancun.yonghu_wangzhi) || daima.huancun.yonghu_wangzhi.StartsWith(args.Uri.AbsoluteUri) || rx.IsMatch(args.Uri.AbsoluteUri) == true)
                    {
                        //检查是否为同一个链接
